...
class DataTrainingArguments:
    ...
    train_file: Optional[List[str]] = field(default=None, metadata={"help": "The input training data file (a text file)."})
    ...
    def __post_init__(self):
        if (
            ...
            if self.test_file is not None:
                if type(self.test_file) == list:
                    extension = self.test_file[0].split(".")[-1]
                else:
...
def main():
    ...
    def preprocess_nq_batch(
        ...
        titles = examples[title_column]

        def generate_input(_question, _context, _title):
            return "  ".join([f"question: {_question.lstrip()}", f"context:{_title.lstrip()}/{_context.lstrip()}"])

        def generate_answer(_answer):
            return _answer if _answer == 'Irrelevant' else f"Answer: {_answer}"

        inputs = [generate_input(question, context, title) for question, context, title in zip(questions, contexts, titles)]
        targets = [generate_answer(answer[0]) if len(answer) > 0 else "" for answer in answers]
        return inputs, targets

    def preprocess_function(examples):
        inputs, targets = preprocess_nq_batch(examples, question_column, context_column, answer_column, title_column='title')
    ...
    def preprocess_validation_function(examples):
        ...
        model_inputs["example_qid"] = []
        model_inputs["example_cid"] = []

        for i in range(len(model_inputs["input_ids"])):
            # One example can give several spans, this is the index of the example containing this span of text.
            model_inputs["example_qid"].append(examples["qid"][i])
            model_inputs["example_cid"].append(examples["cid"][i])
    ...
    def compute_metrics_nq(p: EvalPrediction):
        return compute_em_nq(preds=p.predictions, references=p.label_ids)

    # Post-processing:
    def post_processing_function(
        examples: datasets.Dataset, features: datasets.Dataset, outputs: EvalLoopOutputProb, stage="eval"
    ):
        ...
        example_id_to_index = {(k[0], k[1]): i for i, k in enumerate(zip(examples["qid"], examples["cid"]))}
        print(len(example_id_to_index))
        feature_per_example = {example_id_to_index[(feature["example_qid"], feature['example_cid'])]: i for i, feature in enumerate(features)}
        predictions = {}
        # Let's loop over all the examples!
        for example_index, example in enumerate(examples):
            # This is the index of the feature associated to the current example.
            if example_index == 2758:
                print(example)
            feature_index = feature_per_example[example_index]
            predictions[(example["qid"], example["cid"])] = [decoded_preds[feature_index], probs[feature_index]]

        # Format the result to the format the metric expects.

        formatted_predictions = [{"qid": k[0], "cid": k[1], "prediction_text": v[0], "answer_prob": v[1]} for k, v in predictions.items()]

        references = [{"qid": ex["qid"], 'cid': ex['cid'], "full_answers": ex["full_answers"]} for ex in examples]
    ...
    trainer = QuestionAnsweringSeq2SeqTrainer(
        ...
        compute_metrics=compute_metrics_nq,
