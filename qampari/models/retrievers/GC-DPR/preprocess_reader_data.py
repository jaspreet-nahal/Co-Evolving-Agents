...
if __name__ == "__main__":
    ...
    parser.add_argument("--is_train_set", action='store_true',
                        help="If true, the data will be binarised for train model usage (split into ctx+ and ctx- \
                        and with answer spans selected)")
    parser.add_argument("--retriever_results", required=True, type=str,
                        help="File with retriever results file(json format)")
    parser.add_argument("--out_file", required=True, type=str, help="The file to write serialized results to")
